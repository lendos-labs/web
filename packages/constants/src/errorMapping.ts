import { TxAction, TxErrorType } from '@lendos/types/error';
...
export const getErrorTextFromError = (
  ...
  try {
    const err = error as unknown as { error: { body: string } };
    const parsedError = JSON.parse(err.error.body) as unknown as { error: { message: string } };
